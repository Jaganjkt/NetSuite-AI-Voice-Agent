...
function (serverWidget, runtime, log, url,
    ...
    function handlePost(context) {
        ...
        try {
            ...
            log.error('AVA_Suitelet', e.message + '\n' + e.stack);
    ...
    function processVoiceCommand(body) {
        ...
        try { auditLog.logVoiceInput(transcript, sessionId); } catch (e) { log.error('AVA', 'Audit log failed: ' + e.message); }
        ...
        if (translation.wasTranslated) {
            log.audit('AVA', 'Translated from ' + translation.sourceLanguageName + ': ' + englishTranscript);
        }
...
'  --ava-border: #e0e5ec;',
'  --ava-radius: 12px;',
'  --ava-shadow: 0 2px 8px rgba(0,0,0,0.08);',
...
'  transition: all 0.2s;',
'}',
'.ava-btn-yes { background: var(--ava-success); color: white; }',
'.ava-btn-yes:hover { background: #2d5a31; }',
'.ava-btn-no { background: #f3f4f6; color: var(--ava-text); border: 1px solid var(--ava-border); }',
'.ava-btn-no:hover { background: #e5e7eb; }',
'.ava-btn-open { background: var(--ava-accent); color: white; text-decoration: none; display: inline-block; padding: 7px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; }',
'.ava-btn-open:hover { background: #005bb5; }',
'#ava-controls {',
'  padding: 12px 0 0;',
'  display: flex;',
'  gap: 8px;',
...
'  flex: 1;',
'  padding: 11px 14px;',
'  border: 2px solid var(--ava-border);',
'  border-radius: var(--ava-radius);',
'  font-size: 14px;',
'  outline: none;',
'  transition: border-color 0.2s;',
'}',
'#ava-input:focus { border-color: var(--ava-accent); }',
'#ava-mic {',
'  width: 48px; height: 48px;',
'  border-radius: 50%; border: none;',
'  background: var(--ava-primary); color: white;',
'  font-size: 20px; cursor: pointer;',
'  display: flex; align-items: center; justify-content: center;',
'  flex-shrink: 0; transition: all 0.2s;',
'}',
'#ava-mic:hover { background: var(--ava-accent); }',
'#ava-mic.rec {',
'  background: var(--ava-danger);',
'  animation: avaPulse 1.5s infinite;',
'}',
'#ava-send {',
'  width: 48px; height: 48px;',
'  border-radius: 50%; border: none;',
'  background: var(--ava-accent); color: white;',
'  font-size: 17px; cursor: pointer;',
'  flex-shrink: 0;',
'}',
'#ava-send:hover { background: #005bb5; }',
'#ava-status {',
'  text-align: center;',
'  font-size: 12px;',
'  color: var(--ava-text-muted);',
...
'}',
'.ava-dots { display: inline-flex; gap: 4px; padding: 6px 12px; }',
'.ava-dots span { width: 7px; height: 7px; background: var(--ava-text-muted); border-radius: 50%; animation: avaBounce 1.4s infinite; }',
'.ava-dots span:nth-child(2) { animation-delay: 0.2s; }',
'.ava-dots span:nth-child(3) { animation-delay: 0.4s; }',
'@keyframes avaBounce { 0%,80%,100% { transform: scale(0); } 40% { transform: scale(1); } }',
'@media (max-width: 600px) {',
'  #ava-wrapper { padding: 8px 12px; }',
'  #ava-header h1 { font-size: 18px; }',
'  .ava-msg { max-width: 92%; font-size: 13px; padding: 10px 12px; }',
'  #ava-input { font-size: 16px; }',
'}',
'</style>',
'',
'<div id="ava-wrapper">',
...
'  </div>',
'  <div id="ava-chat">',
'    <div class="ava-msg assistant">',
'      Hello! I am <strong>AVA</strong>, your NetSuite AI Voice Agent.<br><br>',
...
'    </div>',
'  </div>',
'  <div id="ava-status"></div>',
'  <div id="ava-controls">',
'    <input id="ava-input" type="text" placeholder="Type a command or question..." autocomplete="off" />',
'    <button type="button" id="ava-mic" title="Click to speak (Ctrl+M)">&#x1F3A4;</button>',
'    <button type="button" id="ava-send" title="Send">&#x27A4;</button>',
'  </div>',
'</div>',
'',
'<script>',
...
'  "use strict";',
'',
'  var AVA_URL = "' + suiteletUrl + '";',
'  var AVA_SESSION = "ava_" + Date.now() + "_" + Math.random().toString(36).substr(2,9);',
'  var pendingIntent = null;',
'  var busy = false;',
'',
'  var chat = document.getElementById("ava-chat");',
'  var inp  = document.getElementById("ava-input");',
'  var mic  = document.getElementById("ava-mic");',
'  var send = document.getElementById("ava-send");',
...
'    post("process_voice", { transcript: text, sessionId: AVA_SESSION, sourceLanguage: lang });',
'  }',
'',
'  function post(action, data) {',
'    data.action = action;',
'    var x = new XMLHttpRequest();',
'    x.open("POST", AVA_URL, true);',
...
'',
'    var badge = "";',
'    if (r.translated && r.sourceLanguage) badge = \'<div class="ava-lang-badge">Translated from \' + esc(r.sourceLanguage) + "</div>";',
'',
'    if (r.type === "confirmation_required") {',
'      pendingIntent = r.intent;',
'      var c = r.intent.confidence || 0;',
'      var col = c >= 0.85 ? "var(--ava-success)" : c >= 0.6 ? "var(--ava-warning)" : "var(--ava-danger)";',
'      var h = badge + esc(r.message);',
'      h += \'<div class="ava-conf-bar"><div class="fill" style="width:\' + Math.round(c*100) + \'%;background:\' + col + \'"></div></div>\';',
'      h += \'<div class="ava-actions">\';',
'      h += \'<button type="button" class="ava-btn ava-btn-yes" onclick="window._avaYes()">Yes, proceed</button>\';',
'      h += \'<button type="button" class="ava-btn ava-btn-no" onclick="window._avaNo()">Cancel</button>\';',
'      h += "</div>";',
...
'      if (recUrl) {',
'        html += \'<br><br><a href="\' + recUrl + \'" class="ava-btn-open" target="_blank">Open Record</a>\';',
...
'  }',
'',
'  window._avaYes = function() {',
'    if (!pendingIntent || busy) return;',
'    busy = true;',
'    addMsg("user", "Yes, proceed.");',
'    showDots();',
'    post("confirm_action", { intent: pendingIntent, sessionId: AVA_SESSION });',
'    pendingIntent = null;',
'  };',
'  window._avaNo = function() {',
'    if (!pendingIntent) return;',
'    addMsg("user", "Cancel.");',
'    post("reject_action", { intent: pendingIntent, sessionId: AVA_SESSION });',
'    pendingIntent = null;',
'  };',
'',
'  /* ── UI helpers ── */',
...
'  function showDots() {',
'    var d = document.createElement("div");',
'    d.className = "ava-msg assistant"; d.id = "ava-dots";',
'    d.innerHTML = \'<div class="ava-dots"><span></span><span></span><span></span></div>\';',
'    chat.appendChild(d); chat.scrollTop = chat.scrollHeight;',
'  }',
'  function hideDots() {',
'    var d = document.getElementById("ava-dots");',
...
'    h += "<thead><tr>";',
'    ks.forEach(function(k){h += \'<th style="text-align:left;padding:5px 7px;border-bottom:2px solid var(--ava-border);font-weight:600;">\'+esc(k)+"</th>";});',
'    h += \'<th style="padding:5px 7px;border-bottom:2px solid var(--ava-border);"></th></tr></thead><tbody>\';',
'    rows.forEach(function(r){',
'      h += "<tr>";',
'      ks.forEach(function(k){h += \'<td style="padding:5px 7px;border-bottom:1px solid var(--ava-border);">\'+esc(r[k]||"-")+"</td>";});',
'      if(r.recordUrl) h += \'<td style="padding:5px 7px;border-bottom:1px solid var(--ava-border);"><a href="\'+r.recordUrl+\'" target="_blank" style="color:var(--ava-accent);">Open</a></td>\';',
'      h += "</tr>";',
'    });',
'    h += "</tbody></table>";',
'    return h;',
